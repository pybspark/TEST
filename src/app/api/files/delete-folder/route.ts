...
export async function POST(req: NextRequest) {
  ...
  if (updateError) {
    console.error('files delete-folder: files update failed', updateError)
    return NextResponse.json({ error: '폴더 안 파일을 밖으로 옮기지 못했어요' }, { status: 500 })
  }
  ...
  if (deleteError) {
    console.error('files delete-folder: folder delete failed', deleteError)
